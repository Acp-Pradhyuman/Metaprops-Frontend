...
const Filter = ({ show, tab, create, createdApi, id }) => {
	...
	const handleFilter = async () => {
		...
		try {
			const response = await handleApiCall(
				'post',
				`${endpoints.creatorFilter}`,
				data
			);
			// console.log('%%%%%%----->', response?.data?.data);
			if (response?.data?.success) {
				if (tab === 'created') create(response?.data?.data);
			}
		} catch (error) {
						...
						<div className='panel-heading'>
							...
							>
								...
								>
									...
									<i
										className={
											nftDrop ? 'fas fa-angle-up' : 'fas fa-angle-down'
										}
									/>
								</a>
							</h3>
