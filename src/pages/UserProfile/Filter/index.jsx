  ...
  const handleFilter = async () => {
    ...
    try {
      const response = await handleApiCall(
        "post",
        `${
          tab === "Owned" ? endpoints.getUserOwned : endpoints.getFavoritedNft
        }`,
        data
      );
      if (response?.data?.success) {
        if (tab === "Owned") create(response?.data?.data);
        if (tab === "Favorited") fav(response?.data?.data);
      }
    } catch (error) {
